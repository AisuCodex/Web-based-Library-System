...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    try {
        ...
        $stmt = $conn->prepare("SELECT password FROM admin_acc WHERE email = ? LIMIT 1");
        if (!$stmt) {
        ...
        // Update password
        $updateStmt = $conn->prepare("UPDATE admin_acc SET password = ? WHERE email = ?");
        if (!$updateStmt) {
    ...
    <div class="secondContainer">
        <div class="child">
            <div class="backBtn-div">
                <a class="backBtn" href="../admin/adminPage.php" onclick="showLoadingScreen()">X</a>
            </div>

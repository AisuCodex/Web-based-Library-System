...
if (isset($_GET['id'])) {
    ...
    $deleteQuery = "DELETE FROM thesisbooks WHERE id = ?";
    $stmt = $conn->prepare($deleteQuery);
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        echo "<script>alert('Record deleted successfully!'); window.location='ThesisCrud.php';</script>";
    } else {
...
$count_sql = "SELECT COUNT(*) as total FROM thesisbooks";
if ($search !== '') {
    $count_sql .= " WHERE title LIKE '%$search%' OR abstract LIKE '%$search%' OR author LIKE '%$search%' OR year LIKE '%$search%' OR control_number LIKE '%$search%' OR course LIKE '%$search%'";
}
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                <a href="ThesisCrud.php?id=<?php echo htmlspecialchars($row['id']); ?>" onclick="return confirm('Are you sure?')" class="delete-btn">Delete</a>
                </div>

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['code_submit'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM admin_acc WHERE code = ?");
        if ($stmt === false) {
        ...
        if (preg_match('/[^a-zA-Z0-9]/', $newPassword)) {
            ...
            $stmt = $conn->prepare("UPDATE admin_acc SET password = ? WHERE code = ?");
            if ($stmt === false) {
                die("Error preparing statement: " . $conn->error);
            }
...
<head>
  ...
  <title>Admin Reset Password | Capstone/Thesis Management System</title>

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['code_submit'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM assistant_acc WHERE code = ?");
        if ($stmt === false) {
        ...
        if (preg_match('/[^a-zA-Z0-9]/', $newPassword)) {
            ...
            $stmt = $conn->prepare("UPDATE assistant_acc SET password = ? WHERE code = ?");
            if ($stmt === false) {
                die("Error preparing statement: " . $conn->error);
            }
...
<head>
  ...
  <title>Assistant Reset Password | Capstone/Thesis Management System</title>
...
<?php if (!empty($successMessage) && isset($_POST['password_submit'])) { ?>
    ...
    <script>
        ...
        function closeModal() {
            ...
            window.location.href = "../PHP/assistantLogin.php";
        }
    </script>
<?php } ?>

...
error_log("Cancel reservation script started");
...
try {
    ...
    error_log("Processing reservation cancellation: ID=$reservation_id, Type=$type, User=$user_email");
    
    // Determine which table to use based on type
    $table_name = ($type === 'thesis') ? 'thesis_reservations' : 'book_reservations';
    
    // Verify the reservation belongs to the user
    $check_sql = "SELECT id FROM $table_name WHERE id = ? AND user_email = ?";
    ...
    $update_sql = "UPDATE $table_name SET status = 'cancelled' WHERE id = ? AND user_email = ?";
    ...
    if ($update_stmt->affected_rows > 0) {
        error_log("Reservation cancelled successfully: ID=$reservation_id, Type=$type");
        echo json_encode(['success' => true, 'message' => 'Reservation cancelled successfully.']);
    } else {
        error_log("No rows affected when cancelling reservation: ID=$reservation_id, Type=$type");
